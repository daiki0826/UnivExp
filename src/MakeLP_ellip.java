...
public class MakeLP_ellip {

    public MakeLP_ellip(Condition con,double margin){

			try {
				//書き出すlpファイルの定義
				PrintWriter lp = new PrintWriter(new FileWriter(new File(Constant.expPath+"/jobshop_"+String.valueOf(margin)+"σ.qcp")));
				//変数の定義
				int j,k,p,q,PM1,PM2;
				...
				for(j=1;j<=J;j++) {
					for(k=1;k<=Kj[j];k++) {
						lp.println("_C"+Count+": div_"+j+"_"+k+">="+0);
						Count++;
					}
				}
				...
				for(j=1;j<=J;j++) {
					for(k=1;k<=Kj[j];k++) {
						//double PT = con.getMyu(j, k)+"t"*con.getSig(j, k); //marginの値で余力の大きさ決定
						//lp.println("_C"+Count+": CT_"+j+"_"+k+"- ST_"+j+"_"+k+"="+con.getMyu(j,k)+" + "+"["+con.getSig(j,k)+" * t_"+j+"_"+k+"]");
						//lp.println("_C"+Count+": CT_"+j+"_"+k+"- ST_"+j+"_"+k+"="+con.getPT(j,k));
						lp.println("_C"+Count+": CT_"+j+"_"+k+" - ST_"+j+"_"+k+" = pt_"+j+"_"+k);
						Count++;
					}
				}
				...
				for(j=1;j<=J;j++) {
					for(k=1;k<=Kj[j];k++) {
						lp.println("_C"+Count+": div_"+j+"_"+k+" = "+1/(margin*con.getSig(j, k))+" pt_"+j+"_"+k+"-"+con.getMyu(j, k)/(margin*con.getSig(j, k)));
						Count++;
					}
				...
				for(j=1;j<=J;j++) {
					...
					for(k=1;k<=Kj[j];k++) {
						if(k!=Kj[j]) {
							lp.print("[div_"+j+"_"+k+"^2]+");
						}else {
							lp.print("[div_"+j+"_"+k+"^2] <= 1");
						}
					}
					lp.println();
					Count++;
...
//					}
//					for(k=1;k<=Kj[j];k++) {
//						if(k!=Kj[j])lp.print(" + "+con.getPM(j, k));
//						else lp.print(" + "+con.getPM(j, k)+" + ");
//					}
//					for(k=1;k<=Kj[j];k++) {
//						if(k!=Kj[j])lp.print(" + "+con.getPM(j, k));
//						else lp.print(" + "+con.getPM(j, k)+" + ");
...
//				}
				...
				for(j=1;j<=J;j++) {
					for(k=1;k<=Kj[j];k++) {
						lp.println("pt_"+j+"_"+k+">="+con.getSig(j, k));
					}
				}
				lp.println();
